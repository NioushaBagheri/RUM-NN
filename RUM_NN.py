...
class RUM_NN():
    def __init__(this, batch=None, iternum=500, epochs=200, activation=None):
        this.result = None
        ...
        def call(self, inputs, **kwargs):
            ...
            if self.correlation:
                if len(self.formulas) > 4:
                    raise Exception('This model is not able to handel more than four utilities.')
    ...
    def create_model(this, formulaDict, errorDist=None, errorLoc=0, errorScale=1, correlation=False, gamma=1e4):
        ...
        inp = tf.keras.Input(shape=(columnsNum,))
        Utility1=this.Dense1(formula=Formula.formulaList, BetaList=Beta.BetaList, correlation=correlation,
                                 iternum=this.iternum, errorDist=errorDist, errorParam=[errorLoc, errorScale])(inp)

        
        
        class innerlayer(Layer):
            def call(self, inputs):
                mergedOutput = tf.transpose(inputs, perm=[0, 2, 1])

                argmax = tf.nn.softmax(mergedOutput * gamma)
                
                mergedOutput = tf.reduce_sum(argmax, 1)
        
                Sum = tf.reduce_sum(mergedOutput, 1)
        
                return tf.transpose(tf.divide(tf.transpose(mergedOutput), Sum))

        last_layer=innerlayer()(Utility1)
    ...
    def fit_model(this, target):
        ...
        def likelihood(target, output, axis=-1):
        ...
        def scheduler(epoch, lr):
          if epoch < (this.epochs*0.8):
            return lr
          else:
            return lr * tf.math.exp(-0.1)
        
        # calbk2 = tf.keras.callbacks.LearningRateScheduler(scheduler)
        ...
        loglike = this.likelihood(target, output)
        print('Loglikelihood:   ')
        print(loglike)
    ...
    #         Hessian = np.squeeze(Hessian) # Note, in Tensorflow, gradient operator is normalized by length of data
    ...
    #     stds = [invHess[i][i] ** 0.5 for i in range(invHess.shape[0])]
...
if __name__ == '__main__':
    ...
    test = RUM_NN(iternum=2000, epochs=10)
    ...
    F1 = Formula([(W1, a2), (W2, b2), (W3, p2), (W4, q2)], errorWeight=1)

    F2 = Formula([(W1, a1), (W2, b1), (W3, p1), (W4, q1)], errorWeight=1)

    F3 = Formula([(W1, a3), (W2, b3), (W3, p3), (W4, q3)], errorWeight=1)
